...
## 5b. Self-built booking system (replacing Cal.com — not live yet)

A self-built replacement for the Cal.com embed above is built on the
`feature/self-built-booking` branch, previewable at `/booking/new` (not
linked from nav). **`/demo` still shows the Cal.com widget until this is
tested end-to-end and you approve switching it over** — see the plan for the
full cutover checklist.

It syncs bookings to your own Google Calendar (instead of Cal.com's) using a
service account, so no OAuth login flow is needed. One-time setup:

1. In [Google Cloud Console](https://console.cloud.google.com): create or
   pick a project, enable the **Google Calendar API**, then create a
   **Service Account** (IAM & Admin → Service Accounts) and generate a
   **JSON key** for it.
2. From that JSON file: copy `client_email` into `GOOGLE_SERVICE_ACCOUNT_EMAIL`
   and `private_key` into `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY` in `.env.local`
   (keep the `\n` line breaks in the key as literal `\n` — the app unescapes
   them itself).
3. In Google Calendar (the calendar you want bookings to land on): **Settings
   and sharing → Share with specific people → add the service account's
   email → Permission: "Make changes to events."**
4. Copy that calendar's ID from the same Settings page ("Integrate calendar"
   section — for your primary calendar this is just your Gmail address) into
   `GOOGLE_CALENDAR_ID`.

Attendees get a `.ics` calendar file attached to their emails instead — that
works with Apple, Google, and Outlook without needing them to log in to
anything.
...
The self-built booking system (§5b, not live yet) needs a few more columns —
`booking_uid` is now generated by the app itself instead of coming from
Cal.com, self-serve cancel needs somewhere to record when/why, and self-serve
reschedule updates the same row in place instead of creating a new one each
time. Run this once before testing the `feature/self-built-booking` branch:

```sql
create unique index if not exists eliteworker_demo_bookings_booking_uid_idx
  on eliteworker_demo_bookings (booking_uid);

-- One business, one calendar — no two ACTIVE bookings should ever share an
-- exact start time. Backstops the availability check against a request race.
create unique index if not exists eliteworker_demo_bookings_start_time_active_idx
  on eliteworker_demo_bookings (start_time) where status != 'cancelled';

alter table eliteworker_demo_bookings add column if not exists google_event_id text;
alter table eliteworker_demo_bookings add column if not exists cancelled_at timestamptz;
alter table eliteworker_demo_bookings add column if not exists cancel_reason text;
alter table eliteworker_demo_bookings add column if not exists original_start_time timestamptz;
alter table eliteworker_demo_bookings add column if not exists rescheduled_at timestamptz;
alter table eliteworker_demo_bookings add column if not exists reschedule_count integer not null default 0;
```
